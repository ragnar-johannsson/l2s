...
version=$(shell git describe --long --tags origin/master | cut -d - -f 1-1)
...
l2s_$(version)_amd64.deb:
	docker run --interactive --workdir /src --name l2s-build l2s-build /src/build_deb_pkg.sh ${version}
